...
class PRDWC_WooCommerce {
	...
	static function display_custom_fields_simple() {
		...
		if ( $product->has_child() ) {
			return; // already called by woocommerce_before_variations_form
		}
		if ( ! prdwc_is_donation( $post->ID ) ) {
			return;
		}
	...
	static function display_custom_fields() {
		...
		$options    = PRDWC::select_post_options(
			array(
				'post_type' => $project_post_type,
				'orderby'   => 'post_title',
			)
	...
	static function add_to_cart_message( $message, $product_id ) {
		...
		if ( ! empty( $_POST['prdwc-project-name'] ) ) {
			$message = sanitize_text_field( $_POST['prdwc-project-name'] ) . ": $message";
		}
		return $message;
	}
	...
	static function validate_custom_field( $passed, $product_id, $quantity ) {
		if ( $passed && prdwc_is_donation( $product_id ) ) {
			if ( ! empty( $_REQUEST['prdwc-project-id'] ) ) {
				$project = sanitize_text_field( $_REQUEST['prdwc-project-id'] );
			} elseif ( ! empty( $_REQUEST['project-id'] ) ) {
				$project = sanitize_text_field( $_REQUEST['project-id'] );
			} elseif ( ! empty( $_POST['prdwc-project-name'] ) ) {
				$project = sanitize_text_field( $_POST['prdwc-project-name'] );
			...
			if ( prdwc_allow_custom_amount() ) {
				if ( ! empty( $_POST['prdwc-amount'] ) ) {
					$amount = sanitize_text_field( $_POST['prdwc-amount'] );
	...
	static function add_custom_field_item_data( $cart_item_data, $product_id, $variation_id, $quantity ) {
		if ( ! prdwc_is_donation( wc_get_product( $product_id ) ) ) {
		...
		if ( ! empty( $project_id ) ) {
			...
			if ( ! empty( $project ) ) {
				$cart_item_data['prdwc-project-id'] = sanitize_text_field( $project_id );
			}
		} elseif ( ! empty( $_POST['prdwc-project-name'] ) ) {
			$project = sanitize_text_field( $_POST['prdwc-project-name'] );
		} elseif ( ! empty( $_REQUEST['project'] ) ) {
		...
		if ( prdwc_allow_custom_amount() ) {
			if ( ! empty( $_POST['prdwc-amount'] ) ) {
				...
				$amount = sanitize_text_field( $_REQUEST['amount'] );
			} elseif ( ! empty( $_REQUEST['nyp'] ) ) {
				$amount = sanitize_text_field( $_REQUEST['nyp'] );
			} else {
				$amount = null;
			}
